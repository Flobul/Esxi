...
sendVarToJs('jeedomBackgroundImg', 'plugins/Esxi/core/img/panel.jpg');
			...
			<?php
				...
				foreach ($allObject as $object_li) {
					...
					if ($object_li->getId() == $object->getId()) {
						echo '<li class="cursor li_object active" ><a data-object_id="' . $object_li->getId() . '" href="index.php?v=d&p=panel&m=Esxi&object_id=' . $object_li->getId() . '" style="padding: 2px 0px;"><span style="position:relative;left:' . $margin . 'px;">' . $object_li->getHumanName(true) . '</span><span style="font-size : 0.65em;float:right;position:relative;top:7px;">' . $object_li->getHtmlSummary() . '</span></a></li>';
					} else {
						echo '<li class="cursor li_object" ><a data-object_id="' . $object_li->getId() . '" href="index.php?v=d&p=panel&m=Esxi&object_id=' . $object_li->getId() . '" style="padding: 2px 0px;"><span style="position:relative;left:' . $margin . 'px;">' . $object_li->getHumanName(true) . '</span><span style="font-size : 0.65em;float:right;position:relative;top:7px;">' . $object_li->getHtmlSummary() . '</span></a></li>';
					}
				}
			?>
		</ul>
...
if (init('object_id') == '') {
	foreach ($allObject as $object) {
		foreach ($object->getEqLogic(true, false, 'Esxi') as $Esxi) {
			echo $Esxi->toHtml('dview');
		}
	}
} else {
	foreach ($object->getEqLogic(true, false, 'Esxi') as $Esxi) {
	...
	foreach ($child_object as $child) {
		$Esxi = $child->getEqLogic(true, false, 'Esxi');
		if (count($Esxi) > 0) {
			foreach ($Esxi as $Esxi) {
				echo $Esxi->toHtml('dview');
			}
		}
	}
...
<?php include_file('desktop', 'panel', 'js', 'Esxi');?>
