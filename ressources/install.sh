#!/bin/bash
touch /tmp/dependancy_esxi_in_progress
echo 0 > /tmp/dependancy_esxi_in_progress
...
if [ $versionPHP = "5" ]
	then
		...
		echo "*****************************************************************************************************"
		sudo apt-get clean
		echo 20 > /tmp/dependancy_esxi_in_progress
		sudo apt-get update
		echo 80 > /tmp/dependancy_esxi_in_progress
		sudo apt-get install -y php5-ssh2
		echo 100 > /tmp/dependancy_esxi_in_progress
...
if [ $versionPHP = "7" ]
	then
		...
		echo 90 > /tmp/dependancy_esxi_in_progress
		echo 100 > /tmp/dependancy_esxi_in_progress
		echo "*****************************************************************************************************"
		echo "*  Pour finaliser l'installation, redémarrer Jeedom / To complete the installation, restart Jeedom  *"
		echo "*****************************************************************************************************"
...
rm /tmp/dependancy_esxi_in_progress
